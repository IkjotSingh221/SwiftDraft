...
- **Retrieval embeddings are deterministic SYNTHETIC fakes by default, and
  every result is labeled `synthetic=True` all the way into the rendered
  report.** This sandbox has no GPU and no downloaded BGE-M3/BM25 weights.
  `evals/retrieval_eval.py`'s `synthetic_dense_embed` (a curated
  topic-keyword bag, one dimension per topic) and `synthetic_sparse_embed`
  (a literal word-hash bag-of-words) are small, deterministic, and clearly
  named as fakes — they exercise the *real* `ingest.store.hybrid_search`/RRF
  fusion code path against Qdrant's real `:memory:` engine (proving the
  ablation/metric machinery works), but say nothing about real retrieval
  quality. `run_retrieval_eval(live=True)` swaps in the real
  `embed_dense`/`embed_sparse` (wired to `DRAFTFORGE_LIVE=1` in
  `report.py`) for a GPU machine to produce real numbers through the exact
  same code path — untested here, and `report.py` catches any failure from
  it (missing weights, no GPU) and reports that honestly rather than
  crashing the whole harness.

- **Found, not fixed: `ingest.store.get_qdrant_client(":memory:")` does not
  actually create an in-process Qdrant client.** The real `QdrantClient`
  only special-cases the in-memory engine via its `location` constructor
  parameter; `get_qdrant_client(url=None)` only ever forwards its argument
  as `url=`, so `get_qdrant_client(":memory:")` tries (and fails) to parse
  `":memory:"` as a network URL. `tests/test_ingest_store.py` already
  sidesteps this by constructing `QdrantClient(":memory:")` directly;
  `evals/retrieval_eval.py`'s `run_ablation` does the same
  (`client_factory: Callable[[], QdrantClient] = lambda: QdrantClient(":memory:")`)
  rather than editing `ingest/store.py`, which is out of scope for this
  phase. Documented here and in README.md as a real, reproducible finding
  for whoever next touches `ingest/store.py`.

- **Compliance eval fixture drafts are generated by code
  (`evals/compliance_eval.py::_filler_body`/`_build_entries`), not committed
  as static Markdown files**, because `university_thesis.json`'s word ranges
  run into the thousands per section — hand-typing "small" fixture files
  that are also genuinely in-range isn't practical. `_filler_body` cycles a
  fixed word pool to hit an exact target word count (as counted by the real
  `formats.validator.count_words`), so every generated draft's compliance
  outcome is real and reproducible, not asserted.

- **Discovered while building the compliance eval:
  `validate_document`'s tree-completeness check
  (`_check_missing_tree_sections`) and `validate_section`'s own
  `_check_required_subsections` are two independent checks over different
  data**, not one check reused twice. The former requires a separate
  `(SectionSpec, markdown)` entry in `validate_document`'s `sections` list
  for EVERY required node at every depth (e.g. both `ieee_report`'s `method`
  container AND its `method_data`/`method_approach` children need their own
  entries) regardless of what any entry's own text contains; the latter only
  checks whether a container's declared child *titles* appear as headings
  within that SAME container's own provided text. A "compliant" draft under
  the current (unedited — `formats/validator.py` is off-limits this phase)
  validator therefore needs BOTH: a full entry per required node at every
  depth, AND a numbered, title-matching stub heading for each required child
  inlined in its parent's own entry (`_build_entries` does both — see its
  docstring). This has never been exercised end-to-end before this eval,
  because Phase 5's live per-section compliance check only ever calls
  `validate_section` (once per leaf), never `validate_document` — flagged
  as a known gap for Phase 8, not fixed here.

- **Compliance "pass" == zero violations of ANY code, not just a filtered
  error-severity subset**, even though the eval's pass-rate definition is
  phrased in terms of `Severity.ERROR`. Every `Violation` the validator
  constructs today defaults to (and no code path ever overrides)
  `Severity.ERROR` — there is currently no `WARNING` violation anywhere in
  `formats/validator.py` — so the two definitions coincide in practice.
  `run_compliance_eval` still filters on `severity == Severity.ERROR`
  explicitly (rather than `len(violations) == 0`) so this keeps working
  correctly, unchanged, if a future spec/validator change ever introduces a
  non-error severity.

- **Citation faithfulness has two entrypoints, mirroring the
  low-level/high-level split spec.md's other evals use implicitly.**
  `run_citation_faithfulness_eval(sections, ...)` is pure and fully
  injectable (provider/model/retrieve_fn) — the hermetic test's seam, one
  call, no run/project lookup. `run_citation_faithfulness_eval_for_run(run_id, ...)`
  is the harness-facing wrapper: resolves `project_id` + drafted section
  files + leaf briefs' `source_tags`/query from the run's own on-disk state
  (`graph.build_graph.get_run_meta`/`get_state`, `graph.decisions.run_dir`),
  then delegates. It returns `status="not_run"` with an explicit reason
  whenever a real judge isn't actually available (`live=False`, unknown
  run_id, no drafted sections, or a live call that raises) — never a
  fabricated rate, per spec.md's working-style rule.

- **Sampling is `random.Random(seed).sample(...)`, deterministic per seed,
  default `sample_size=20`.** A fixed seed (default 42) makes a report
  reproducible across runs of the same draft — re-running the harness against
  an unchanged run produces the same spot-check sample, which matters for
  the human-spot-check protocol (a reviewer auditing the same claims each
  time, not a shuffled new set).

- **The report writes to two locations, both documented and both tested**:
  always `{data_dir}/evals/eval_report.md` (a standing, global report,
  independent of any one run), and — only when `--run <run_id>` names a
  KNOWN run — additionally `data/runs/{run_id}/artifacts/eval_report.md`,
  which is exactly the path `render/pandoc.py::ARTIFACT_SPECS["eval_report.md"]`
  already serves via `GET /runs/{id}/artifacts/eval_report.md` (Phase 6's
  slot). No route or `ARTIFACT_SPECS`/Downloads.tsx change was needed —
  `list_artifacts` already marks an entry `available` purely by
  `path.exists()`, so writing the file at that exact path is the entire
  integration.

- **Retrieval's labeled fixture is data
  (`tests/fixtures/evals/retrieval_fixture.json`), loaded by production code
  in `src/draftforge/evals/`.** Every other fixture this phase ships
  (compliance drafts) is generated in code instead (see above), but the
  retrieval labeled set — a list of chunks and query→relevant-chunk-ids
  pairs — is naturally data-shaped, so it's checked in as JSON like the
  task instructions describe ("fixtures under tests/fixtures/evals/") rather
  than duplicated into `src/`. Referencing `tests/fixtures/` from production
  code is a one-off exception to the usual `src`/`tests` separation,
  acceptable here because the harness only ever runs from a repo checkout
  (`make eval`), never from a packaged/installed wheel.
